...
class RepairsAndMaintenanceEntryController extends GetxController {
  ...
  TextEditingController nextDueController = TextEditingController();
  ...
  void play() {
    if (recordFilePath != null && File(recordFilePath!.value).existsSync()) {
      ...
      audioPlayer.setUrl("${ApiUrl.baseUrl}expense/${userDataProvider
          .getRepairsAndMaintenanceData!.audioFile}");
      audioPlayer.play();
    }
  }
  ...
  subCategory1Call(String maintenance) async {
    Map<String, dynamic> payload = {
      'ExpenseCategoryId': userDataProvider.expenseId.toString(),
      'Sub1CategoryName': maintenance,
    };
  ...
  Future<void> fetchSubCategoryTwo(
    ...
    print('fetchSubCategoryTwoPayload:$payload');
  ...
  Future<void> insertMaintenance() async {
    if (firstValidation()) {
      Map<String, String> payload = {
        ...
        'purchaseId': userDataProvider.getInsertPurchaseData ?? "",
      };
      print("insertMaintanceRequest:$payload");
      uploadLoading.value = true;

      var response = await _connect.imgTaskUpload(ApiUrl.insertMaintenance,itemImagePick.value.file , payload, recordFilePath!.value ?? "");
      uploadLoading.value = false;
      debugPrint("insertMaintanceResponse: ${response.toJson()}");
      if (!response.error!) {
        Fluttertoast.showToast(
        ...
        Get.offNamed(AppRoutes.repairsAndMaintenanceSummaryScreen.toName);
      } else {
        Fluttertoast.showToast(
          msg: response.message!,
          toastLength: Toast.LENGTH_SHORT,
  ...
  Future<void> updateMaintenance() async {
    ...
    Map<String, String> payload = {
      ...
      'Comments': commentController.text,
      'ExpenseDate': currentDateController.text,
      'RepairsMaintenanceExpenseId':
          userDataProvider.getRepairsAndMaintenanceData!.id.toString(),
    ...
    print("updateMaintanceRequest:$payload");
    var response = await _connect.imgTaskUpdate(ApiUrl.updateMaintenance, itemImagePick.value.file , payload, recordFilePath!.value ?? "");
    debugPrint("updateMaintanceResponse: ${response.toJson()}");
    if (!response.error!) {
      Fluttertoast.showToast(
        msg: response.message!,
        toastLength: Toast.LENGTH_SHORT,
  ...
  void setData() {
    RepairsAndMaintenanceData data =
    userDataProvider.getRepairsAndMaintenanceData!;
    subCategory1Controller.text = data.category2 ?? "";
    subCategory2Controller.text = data.category3 ?? "";
    nextDueController.text = data.nextDueDate ?? "";
