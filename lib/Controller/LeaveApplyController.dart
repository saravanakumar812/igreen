...
class LeaveApplyController extends GetxController {
  ...
  RxBool leaveReasonDropDown = RxBool(false);
  ...
  bool firstValidation() {
    if (leaveTypeController.value.text.isEmpty) {
  ...
  Future<void> applyLeave() async {
    if (firstValidation()) {
      Map<String, String> payload = {
        ...
        'explanation': messageController.text,
        'from_date': fromDateController.text,
        'to_date': toDateController.text,
        'EmployeeId': AppPreference().getEmpId.toString(),

      };
      print("applyLeave:$payload");
      var response = await _connect.leaveApply(ApiUrl.applyLeave, payload, recordFilePath!.value);
      debugPrint("applyLeave: ${response.toJson()}");
      if (!response.error!) {
        Fluttertoast.showToast(
        ...
        Get.offNamed(AppRoutes.leaveStatusScreen.toName);
      }
    }
  }
