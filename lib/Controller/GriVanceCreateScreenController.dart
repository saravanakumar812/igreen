...
class GriVanceCreateScreenController extends GetxController {
  ...
  Rx<PickedImage> itemImagePick = Rx<PickedImage>(PickedImage
    (imagePath: null, file: null, renderType: RenderImage.platform, extension: '', base64: ''));
  ...
  Future<void> addEscalationForm() async {
    if (firstValidation()) {
      Map<String, String> payload = {
        'EmployeeId': AppPreference().getEmpId.toString(),
        'Messages': messageController.text,
        'employeeName': selectPersonController.text

      };
      messageController.clear();
      print("addForm:$payload");
      var response = await _connect.imgTaskUpload(ApiUrl.addEscalation, itemImagePick.value.file , payload, recordFilePath!.value ?? "");
      debugPrint("addResponse: ${response.toJson()}");
      if (!response.error!) {
        Fluttertoast.showToast(
        ...
        Get.offNamed(AppRoutes.griVance.toName);
      }
    }
  }
}
